...
6. **赢得比赛需要的最小训练时长**：所需要的**经验值**和**体力值**分开讨论，经验值采用模拟法推演，体力值推理可以得到，为所有消耗的体力值之和+1-初始体力值（注意对负数结果的处理）
...
class Solution:
    def hasCycle(self, head: Optional[ListNode]) -> bool:
        '''
        思路：将所有的next指向的node放在一个列表中，如果遍历到某个几点，其next已经在列表中，则有环
        '''
        all_list = []
        pin = head
        while pin:
...
class Solution:
    def minNumberOfHours(self, initialEnergy: int, initialExperience: int, energy: List[int], experience: List[int]) -> int:
        '''
        最后获得胜利，就要打败最后一个[-1]对手
        要打败[-2]的选手，
        - myenergy[-2] = myenergy[-1] + energy[-2]
        - myenergy[-n] = myenergy[-1] + energy[-2] + ... + energy[-n] = sum(energy[i])+1
        '''
        addenergy = sum(energy) + 1 - initialEnergy if sum(energy) + 1 > initialEnergy else 0
        # 模拟推演
        addexperience = 0
        myexperience = initialExperience
        for i, exp in enumerate(experience):
            if myexperience <= exp:
                tmp = exp - myexperience + 1 # 需要额外补充的
                myexperience += exp + tmp # 本次打完后剩余的
                addexperience += tmp
            else:
                myexperience += exp
